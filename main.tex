\documentclass[14pt]{extarticle}

\input{preamble.tex}
\input{hyphenation.tex}

\hypersetup{final}

\begin{document}

% \include{title}


\newpage
\setcounter{page}{2}

\vspace*{\fill}
\begin{abstract}
Данная работа посвящена вопросу экономии видеопамяти, потребляемой в процессе отрисовки кадра графического приложения реального времени промежуточными структурами данных алгоритмов визуализации сцен.
Широко распространившийся подход структурирования вычисления кадра в виде графа позволяет более качественно управлять аллокациями видеопамяти, а также автоматизировать различные аспекты разработки современных трёхмерных приложений.
В рамках работы предложено комплексное решение задания кадровых графов, вопрос управления памятью временных ресурсов сведён к задаче дискретной оптимизации, а также предложена качественная, эффективная схема её решения.
Преимущество предложенной схемы над существующими заключается в переиспользовании памяти, потребляемой ресурсами темпоральных алгоритмов, широко распространённых среди современных методов визуализации трёхмерных сцен.
\end{abstract}
\vspace*{\fill}

\newpage
\tableofcontents
\newpage

\include{chapters/intro}
\include{chapters/domain}
\include{chapters/problem}
\include{chapters/overview}
\include{chapters/proposed}
\include{chapters/results}
\include{chapters/conclusion}

\newpage

\emergencystretch=3em
\printbibliography
\end{document}

